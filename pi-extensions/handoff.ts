...
function buildHandoffContext(entries: SessionEntry[], goal: string): HandoffContext {
	...
	const relevantFiles = uniqueLimited(extractFileCandidates(allText.join("\n")), MAX_RELEVANT_FILES);
...
function getSection(markdown: string, heading: (typeof REQUIRED_HEADINGS)[number]): string | undefined {
...
function enforceSchema(draft: string, context: HandoffContext): string {
	...
	const contextSection = extracted["Context"] ?? (summaryFallback || "- Continue from the prior implementation discussion.");
	const decisions = extracted["Decisions Made"] ?? "- Preserve previously discussed constraints and architecture choices.";
	const relevantFiles =
	...
	const implementationPlan =
		...
		["1. Reconfirm current code state in relevant files.", "2. Implement the Objective with minimal, focused changes.", "3. Validate behavior and summarize what changed."].join("\n");
	const acceptanceCriteria = ensureAcceptanceChecklist(extracted["Acceptance Criteria"] ?? "");
	const risks = extracted["Open Questions / Risks"] ?? "- Confirm any ambiguous requirements before implementation.";
