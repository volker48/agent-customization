/**
 * Reusable web fetch core.
 *
 * Shared by the standalone `webfetch` tool and the Fusion `webfetch` tool so both
 * have identical fetch semantics, including SSRF/private-host protection, redirect
 * handling, and HTML-to-markdown conversion. The standalone tool owns the parameter
 * schema and registration; this module owns the fetch/convert/truncate logic.
 *
...
/**
 * Fetch an HTTP(S) URL and return text-like content as a tool result.
 *
 * Applies SSRF/private-host protection, redirect following, optional HTML-to-markdown
 * conversion, probe/smart strategies, and output truncation.
 *
 * @param params Fetch parameters (url plus optional mode/strategy/headers/maxChars).
 * @param signal Optional abort signal for cancellation.
 * @returns Tool result content plus structured fetch details.
 */
  ...
  try {
    ...
    for (const candidate of alternateCandidates) {
      ...
      try {
        ...
        smartNotes.push(`Skipped invalid alternate URL from ${candidate.source}: ${candidate.url}`);
        continue;
      }
